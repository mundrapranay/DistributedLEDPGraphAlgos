		...
		for _, eps_t := range epsilons {
			for run_id := 0; run_id < exp_config.Runs; run_id++ {
				...
				for bf := exp_config.Bias_Factor; bf <= exp_config.Bias_Factor; bf++ {
					outputFile = fmt.Sprintf("/home/pm886/palmer_scratch/results/%s_%s_%.2f_%d_%d_%d_%d_%d_%.2f_%s.txt", graph, exp_config.AlgoName, factor, b2i[exp_config.Bias], b2i[exp_config.Noise], bf, run_id, exp_config.Num_Workers, eps_t, exp_config.ExpTag)
